...
- **Shorts proposer**: `python3 tools/publish/shorts_proposer.py <slug> [--top N] [--emit-manifest] [--json]` — scans the assembly manifest for high-signal moments that make good standalone Shorts (TEMPLATE/DataChart + StatCaption → StatRevealShort, TEMPLATE/Quote → KineticShort, TEMPLATE/FrameworkDiagram → FrameworkDiagram-Short, plus beat-opener candidates from the first 30s of each beat). Scores each on duration sweet-spot (25-45s), priority, and anchor presence; top-N proposed. `--emit-manifest` writes a skeleton `episodes/<slug>/shorts-manifest-proposed.json` with TODO markers for the operator to fill in (template-specific `data` blocks need script-level understanding the tool doesn't have). Beat-opener candidates with no anchor visual AND below-band duration are skipped — a 30s talking-head opener isn't a Short worth proposing. Warns to stderr when a TEMPLATE component isn't in the proposer's mapping table (caught fallbacks would otherwise mis-route silently).
