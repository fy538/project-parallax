...
export const HypothesisTrackSchema = z.object({
  label: z.string(),
  prior: z.number().min(0).max(100),
  color: z.string().optional(),
});

export const MultiHypothesisSchema = z.object({
  label: z.string(),
  prior: z.number().min(0).max(100),
  color: z.string().optional(),
});

export const BayesianUpdateDataSchema = z.object({
  episode: z.string(),
