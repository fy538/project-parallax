 ...
 * canonical pattern). Schema closure: added `RoutePhase.camera` (longitude/
 * latitude/zoom/pitch/bearing) — the .tsx already consumed `phase.camera`
 * but the schema previously omitted it, so Zod stripped the field at parse
 * time. See RouteAnimation.tsx ~L221.
 */
...
export const RoutePhaseCameraSchema = z.object({
  longitude: z.number(),
  latitude: z.number(),
  zoom: z.number(),
...
export const RoutePhaseSchema = z.object({
  title: z.string(),
  ...
  camera: RoutePhaseCameraSchema.optional(),
});

export const RouteRadialSchema = z.object({
...
export const RouteInsetSchema = z.object({
...
export const RouteAnimationDataSchema = z.object({
  episode: z.string(),
  title: z.string(),
  subtitle: z.string().optional(),
  ...
  radial: RouteRadialSchema.optional(),
  center: z.tuple([z.number(), z.number()]).optional(),
  scale: z.number().optional(),
   ...
   */
  inset: RouteInsetSchema.optional(),
