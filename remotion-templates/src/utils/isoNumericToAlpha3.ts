 ...
 *
 * Why this file exists: the `world-atlas` TopoJSON file ships country
 * features keyed by ISO 3166-1 **numeric** codes ("840" for USA, "156" for
 * China). All other Parallax data and components key on ISO 3166-1
 * **alpha-3** codes ("USA", "CHN"). This lookup is the bridge.
 *
 * Source: ISO 3166-1 (stable list maintained by ISO 3166/MA). The 249
 * codes below are the complete current set as of 2023; world-atlas
 * countries-110m.json uses ~177 of them (skips small dependencies and
 * micronations). Codes are stable — additions in the last 30 years are
 * rare (South Sudan: 728/SSD in 2011; Curaçao: 531/CUW in 2010).
 *
 * Maintenance: when ISO adds a new code, append it here. Removed codes
 * (e.g., "AN"/"ANT" Netherlands Antilles, dissolved 2010) are kept as
 * legacy entries so old data still resolves.
 *
 * Not exported as a Map because the lookup is read-only and a plain
 * object's V8 hidden-class lookup is faster for this size.
 */

/** Numeric code string → ISO 3166-1 alpha-3 code. */
export const ISO_NUMERIC_TO_ALPHA3: Readonly<Record<string, string>> = {
...
/**
 * Normalize a world-atlas feature ID (sometimes zero-padded, sometimes
 * not — Topojson encoders vary) to an alpha-3 code. Returns null when
 * the numeric code is unknown.
 */
export const numericIdToAlpha3 = (id: string | number): string | null => {
  const s = String(id).padStart(3, "0");
  return ISO_NUMERIC_TO_ALPHA3[s] ?? null;
};
