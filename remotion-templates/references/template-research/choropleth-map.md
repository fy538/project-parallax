...
2. ~~**ColorBrewer-vetted defaults.** Build palette options around `RdBu` (diverging) and `YlOrBr` rust-to-bone (sequential), bake into the template rather than letting per-episode data pick arbitrary colors.~~ **Done — May 11, 2026.** Shipped two new ramp names: `ylOrBr` (sequential warm: paper → bone → gold → rust → oxblood) and `rdBu` (diverging: oxblood → bone midpoint → blue). Brand-aligned variants of the ColorBrewer 5-class schemes. `ylOrBr` is now the default ramp when `colorRamp` is unspecified (was `blue`). Type docstring names the editorial rule: use `rdBu` only when data has a meaningful midpoint; use `ylOrBr` for quantitative-only data.
...
5. ~~**"No data" treatment.** Hatched or stippled fill for missing regions so it's distinguishable from the lightest bin.~~ **Done — May 11, 2026.** Shipped as `country.noData?: boolean` on `CountryData`. When set, the country renders in a distinct umber neutral (`palette.umber`) rather than the ramp's gray — explicitly chosen to be distinguishable from the lightest bin of the active ramp. Sprite-based hatching would have required a Mapbox sprite asset and added build complexity; the umber neutral satisfies the dossier intent (distinguishable from "low value") within the existing fill-expression pipeline.
