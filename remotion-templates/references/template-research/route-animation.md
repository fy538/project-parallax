...
2. ~~**Path-style audit.** Current routes may be using saturated colors; default should be ink + rust-for-current-segment.~~ **Done — May 11, 2026.** Settled segments now render in `palette.ink` (subdued context); the current-phase animating segment uses the accent `routeColor`. Override via `data.routeColor` for episodes that genuinely need a single saturated color across all segments (rare — usually per-state styling is what makes a multi-route map readable). `seg.color` and `point.color` per-data overrides still take precedence.
...
4. ~~**Easing review.** Confirm ease-out cubic with 300ms hold between segments; flag any linear easing.~~ **Done — May 11, 2026.** Audited `RouteAnimation.tsx` interpolations: camera transitions use `CLAMP_CUBIC_INOUT`, arc-progress reveal uses `CLAMP_CUBIC`, intra-phase progress uses `CLAMP` with documented `// linear-ok` annotation (consumed by camera/bearing-drift which apply their own easing). Polish-lint A1 confirms — no naked linear interpolations.
