...
3. ~~**Small-multiples sub-variant** (`variant: "small-multiples"`) when series count exceeds 4. Shared y-axis, sparse labeling, hero panel highlighted.~~ **Done — May 11, 2026.** Shipped as `variant: "small-multiples"`. Auto-grid layout (3 columns by default). Each panel renders one line against the shared y-domain with a tiny baseline + terminal value label. Hero panel (`line.hero: true`) gets accent color + heavier stroke (3px vs 1.75px) + bolder terminal label. Catalog reference: `catalog-time-series-chart-population-small-multiples` (Asia hero + Africa/Europe/Americas — same Y-axis exposes the scale of Asia's lead).
4. ~~**Annotation label hierarchy.** Annotation labels should describe *why*, not *what* — "OPEC embargo," not "CO₂ rises sharply." Audit existing annotations to enforce.~~ **Done — May 11, 2026.** Codified inline on `TimeSeriesAnnotation.label` JSDoc with the right/wrong examples and the underlying rule: *"the chart shows what — your label exists to name the cause or the moment of recognition. If your label just describes the visible shape of the line, delete it."* Audited current catalog annotations: clean (no offending "what" descriptions in episode data).
