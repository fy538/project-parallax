...
- ✅ **Adopted in `TitleTransition.editorial-title`** (May 11, 2026, commit 1da5a49): the title and dek reveals use `anticipatoryStartFrame()` to compute their start frames from `_direction.syncPoints[0]` (title cue) and `_direction.syncPoints[1]` (dek cue), with fallback to the original estimate start frames when no sync points are present. Uses `anticipatoryStartFrame() + fadeIn()` rather than `anticipatoryReveal()` directly — preserves the linear fade curve so the fallback path produces identical baselines while still landing 150ms before the narrator's word when sync points exist.
- ✅ **Five more templates adopted directly** (May 11, 2026): `StatReveal` (count-up completion lands pre-cue, commit e9b4bd3), `KineticTypography` × 4 variants (quote/definition/bilingual hero text + statistic count-up, commit 2029c66), `BayesianUpdate` (question card lands pre-narration, commit aeee863), `TitleTransition` non-editorial variants × 3 (episode-title title+subtitle, section sectionTitle, end-card ctaText; commit b808ac7).
- ✅ **TitleBlock shared-component adoption** (May 11, 2026, commit TBD): the shared `<TitleBlock>` component now accepts an optional `syncPoints` prop and anticipates the block's entrance via `syncPoints[0]`. **Fans out to all 25 TitleBlock consumers** — every template that uses `TitleBlock` for its title gets D17 coverage automatically: AnnotatedImage, AtlasPlate, BayesianUpdate, BifurcationRoute, CartogramMap, ChoroplethMap, DataChart, DecisionTree, DualTimeline, DuelingFrameworks (Static + Cinematic), EscalationLadder, FrameworkDiagram, GameBoard, HorizontalTimeline, NetworkDiagram, PricingWaterfall, ProbabilityGauge, ProportionalSymbolMap, RadarChart, RouteAnimation, SankeyFlow, StatReveal, StrategicLandscape, TimeSeriesChart. The TitleBlock-level adoption is **block-level only** (title + subtitle land together via the single FadeIn wrapper); templates needing fine-grained title-vs-subtitle anticipation (TitleTransition.editorial-title) keep their split per-element pattern.
- **D17 surface area covered**: 5 piecemeal + ~25 via TitleBlock fan-out = ≥ 30 templates wired (with overlap — some templates adopt both their own hero plus inherit TitleBlock's title anticipation). The pattern is now dominant in the codebase.
- **Activation gating:** every adoption is dormant until the manifest pipeline (`tools/assembly/generate_manifest.py`) starts emitting `_direction.syncPoints` from Whisper-aligned narration. Until then, fallback paths produce byte-identical baselines.
