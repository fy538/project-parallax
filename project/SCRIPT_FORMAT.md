...
The right column carries seven content-type mode tags — `[FOOTAGE:]`, `[MG:]`, `[LAYERED:]`, `[AI-GEN:]`, `[ILLUST:]`, `[SCENE:]`, `[ARCHIVAL:]` — plus three special-purpose tags: `[FORECAST:]` (probability gauge), `[BACKDROP: id]` (cascade backdrop selection), `[OVERLAY: preset]` (rare per-segment FilmOverlay override). Conceptually the content-type tags collapse to **three content types** (per VISUAL_LANGUAGE.md → "The Three Content Types"): Remotion (analytical), AI-generated (constructivist illustration in atmospheric or grounded role), and Footage (real-world capture — archival or screen recording). The three-type model is the simpler mental map for script-drafting; the seven tags below encode editorial role distinctions even when the underlying content type is shared.
...
- **Visual mode tags are mandatory.** Every right-column entry should carry one of the seven content-type tags — `[FOOTAGE:]`, `[MG:]`, `[LAYERED:]`, `[AI-GEN:]`, `[ILLUST:]`, `[SCENE:]`, `[ARCHIVAL:]` — and may additionally carry the special-purpose tags `[FORECAST:]`, `[BACKDROP: id]`, `[OVERLAY: preset]`. These tags route work to the correct downstream tool and enable automated pacing checks. See the Visual Modes section above for full definitions.
