...
| 07 | Audio-bridged cut | **not a `TransitionType`** — `narrationLeadIn` / `narrationLagOut` fields | 0.5–1.0s overlap | ✅ Phase 7 — default `narrationLeadIn: 0.7` on every hard cut |
...
- ✅ **Phase 5 (revise `apply_default_transitions` Rule 4, shipped May 16, 2026)**: within-beat template→template fallback flipped from `dissolve` (0.3s) to **hard cut** (0s). The Rule 4a match-cut path (Phase 4) is unaffected. Dissolve is now opt-in via `_direction.transitionOut: "dissolve"` on the prior segment (visual-spec) or `DIR: cut(dissolve)` (script). Rule 7's breathing-pace upgrade still promotes cut→dissolve (0.4s), which IS the doctrinal escalation criterion. Audit of committed manifests found 8 within-beat dissolves that were the implicit Rule 4 output — they remain in checked-in manifests for now; next visual-spec regeneration will pick up the new default, and visual-spec should re-emit explicit DIR overrides for the 2-3 cases (KineticTypography→KineticTypography elaboration) that genuinely warrant dissolve.
