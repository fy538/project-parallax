...
**Script version:** v6.3 (May 17, 2026) — **Engagement-first restructure** per research-driven editorial doctrine. Audit-passed + A/B-validated against v5.6 (blind reviewer picked v6 on all 5 dimensions; structural metrics flagged 2 regressions, of which the pattern-interrupt drop is addressed in v6.3):
- v6.2 → v6.3: pattern-interrupt density **partial** mitigation. The 12s cold-open AI-GEN scene (Beat 1, RAND research office) is split into three distinct visual moments — wide establishing shot (5s, push-in) → KineticTypography lower-third caption "RAND CORPORATION · JANUARY 1950" (2s, fade-in) → detail shot of the paper grids (5s, drift). Same constructivist still; one new Pika clip (~$0.30) for the detail-shot drift. Pattern interrupts move from 5.0/min → 5.1/min — small improvement. **The remaining gap to v5's 8.9/min is structural and intentional:** v6's bridge sentences + atmospheric register shifts in Beats 1-2 are the engagement-first restructure's whole point; restoring v5's interrupt density would require cuts every 4-5s that contradict the calm-confident editorial register the blind A/B reviewer specifically validated as v6's strength. **The metric is a correlate, not a cause** — the blind reviewer picked v6 on every dimension DESPITE the lower interrupt density. v6.3 closes the cheap-and-aligned subset of the regression; the rest is editorial choice, not bug. See `episodes/prisoners-dilemma/v5-vs-v6-ab-review.md` for the full A/B finding.
Earlier evolution:
...
| In January 1950, two researchers at the RAND Corporation ran the first-ever prisoner's dilemma experiment. {✅} | **P3** · [AI-GEN:] scene · "Constructivist mid-century research office: WIDE establishing shot — two geometric figures in shirtsleeves at a small table with paper grids, warm amber light through venetian-blind slats casting parallel shadows, color-blocked interior in bone and grey-green" · vector_illustration · standard · 5s · Pika: slow push-in toward the table |
|  | DIR: cam(push-in, over:5s) |
|  | DIR: mood(subtle, drift:slow) |
| | **P3** · [LAYERED:] AI-GEN scene continues underneath + KineticTypography overlay: "RAND CORPORATION · JANUARY 1950" — lower-third caption, amber on muted ink panel, fades in over 0.5s · 2s |
|  | DIR: reveal(fade-in, over:0.5s) |
|  | DIR: hold(land) |
| The subjects — an economist named Armen Alchian and a mathematician named John Williams — played one hundred rounds of the game that was supposed to prove cooperation is irrational. {✅} | **P3** · [AI-GEN:] scene · "Constructivist research office DETAIL — close-up on the paper grids on the small table, two geometric hands visible at edges of frame, pencil marks tracking the cooperation/defection outcomes round-by-round, warm amber from venetian-blind shadow falling across the page" · vector_illustration · standard · 5s · Pika: slow drift across the page surface, pencil marks appearing one by one |
|  | DIR: cam(static, focus:grid) |
|  | DIR: mood(subtle, drift:slow) |
