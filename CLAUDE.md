...
**Shipped May 18, 2026 (math-rendering register, Phases 1–4):**
- **Math-rendering register** — 6 canonical primitives for editorial mathematics: Reveal (single-equation write-on), Step-by-step (multi-step derivation crossfade), Term-highlight (`\textcolor{HEX}{TERM}` in TeX), Substitution (variable → value step), Annotation (KaTeX `\underbrace` / `\overbrace`), Inline (`<MathInline>` parsing `$...$` in prose). Plus the composite Bounded form — Parallax's signature analogical move applied to equations. Doctrine: [`project/MATH_RENDER_REGISTER.md`](./project/MATH_RENDER_REGISTER.md). Components: `MathExpression`, `MathCrossfade`, `MathInline`. Templates: `MathReveal`, `MathDerivation`. Visual reference: `catalog-showcase-math-render` (sequential tour through all six primitives). Lint: `M-MATH-VALID` validates every TeX formula through KaTeX strict parser. 56 unit tests; visual-spec skill updated. Renders on KaTeX bundled fonts (Computer-Modern-derived) for Phase 1; Latin Modern Math font swap queued for Phase 5.
- **Editorial centralization sweep** — five new single-source-of-truth tokens shipped with paired Python + TS lint rules: brand mark glyph (`palette.json::brandMark` → theme.ts + brand.py, `no-literal-brand-mark`), z-index global scale (8 semantic tiers in theme.ts, `no-bare-zindex-on-global-scale`), status emoji (🟢/🟡/🔴 in tools/status_emoji.py, `no-bare-status-emoji`), video defaults (`tools/config/video.json` → theme.ts + 4 Python CLIs), episode-label format (`formatEpisodeLabel` in theme.ts). Every refactor format-preserving — no pixel drift.

**Shipped May 17–18, 2026:**
- **DecisionTree aesthetic exploration — 5 variants + sibling template.** Research-driven extension of the decision-tree form (research report 2026-05-17). DecisionTree now supports `extensive` (default), `ladder`, `indented` (manuscript-outline / FT Visual Vocabulary "Hierarchy" register), `spine` (Kahn-ladder-derived stem-and-leaf), and `schematic` (engineering-drawing boxed nodes + orthogonal edges). Pre-existing `ladder` variant got a POLISH.md D1 polish pass (per-option borders removed; left-rail accent only). New separate template `OutcomePartition` for the "decision space narrows" register (RAND-style partition map with named axes + recursive subdivision). All variants registered in `FullEpisode.tsx` TEMPLATE_COMPONENTS, schema enum, catalog showreel (Scenarios section), and dossiers (`references/template-research/decision-tree.md` + new `outcome-partition.md`). PARALLAX_VISUAL_VOCABULARY.md § 10 carries the variant chooser; DIAGRAM_TEMPLATE_SELECTOR.md adds the DT-variants table + DT-vs-OutcomePartition sibling-disambiguation. visual-spec skill updated. EscalationLadder doctrine-compliance fix (heat-gradient panel removed). Episode runtime impact: +64s on Showreel.
...
- [`project/MATH_RENDER_REGISTER.md`](./project/MATH_RENDER_REGISTER.md) — six canonical math-rendering primitives (Reveal, Step-by-step, Term-highlight, Substitution, Annotation, Inline) plus the composite Bounded form. Sibling of TEXT_ANIMATION_REGISTER and HOLD_MOTION_REGISTER for moments where the editorial argument is quantifiable and the equation IS the load-bearing piece. Wired via `MathReveal` / `MathDerivation` templates, `<MathInline>` for prose-embedded math, KaTeX-native annotations (`\underbrace`/`\overbrace`), `\textcolor{HEX}{TERM}` brand-palette highlights, the `M-MATH-VALID` formula-parse lint, and the `catalog-showcase-math-render` sequential reference card.
