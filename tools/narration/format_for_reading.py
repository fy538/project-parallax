...
def render_read_doc(
    ...
    lines: list[str] = [
        f"# Narration — {slug}",
        "",
        f"> Auto-generated by `tools/narration/format_for_reading.py`. **Do not hand-edit** —",
        f"> regenerate from the production script after every script change.",
        f">",
        f"> Read at ~{wpm} wpm. `///` marks sentence boundaries (breath cues).",
        f"> _Italic lines_ are stage directions — do **not** read aloud.",
        f"> `[BEAT]` / `[PAUSE]` lines mean a deliberate silence — count one or two seconds.",
        "",
